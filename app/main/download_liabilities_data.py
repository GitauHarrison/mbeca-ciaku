...
def download_liabilities_pdf(user):
    # PDF()
    ...
    # Get a list of the amounts spent throughout the liabilities years
    amounts = [liabilities_date.amount for liabilities_date in liabilities]

    # Get a list of the liabilities items in the liabilities years
    items = [liabilities_date.name for liabilities_date in liabilities]
    ...
    # --- table showing liabilities items and amounts spent in each year ---
    ...
    if session['year'] in months_in_year.keys():
        ...
        pdf.output(
            current_app.config['PDF_FOLDER_PATH'] + 'liabilities_data' + session["year"] + '.pdf', 'F')
