...
def download_expenses_pdf(user):
    # PDF()
    ...
    # Get a list of the amounts spent throughout the expenses years
    amounts = [expenses_date.amount for expenses_date in expenses]

    # Get a list of the expenses items in the expenses years
    items = [expenses_date.name for expenses_date in expenses]
    ...
    # --- table showing expenses items and amounts spent in each year ---
    ...
    if session['year'] in months_in_year.keys():
        ...
        pdf.output(
            current_app.config['PDF_FOLDER_PATH'] + 'expenses_data' + session["year"] + '.pdf', 'F')
