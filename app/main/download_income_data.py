...
def download_income_pdf(user):
    ...
    # Get a list of the amounts spent throughout the incomes years
    amounts = [incomes_date.amount for incomes_date in incomes]

    # Get a list of the incomes items in the incomes years
    items = [incomes_date.name for incomes_date in incomes]
    ...
    # --- table showing incomes items and amounts spent in each year ---
    ...
    if session['year'] in months_in_year.keys():
        ...
        pdf.output(
            current_app.config['PDF_FOLDER_PATH'] + 'income_data' + session["year"] + '.pdf', 'F')
